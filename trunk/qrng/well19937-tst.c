...
int main(int argc, char *argv[])
{
	...
	unsigned int init_seed[R];				// Init WELL19937
	...
	for(i = 0; i < sizeof(init_seed) / sizeof(init_seed[0]); i++) printf("0x%0X, ", init_seed[i]);
	printf("\n");
        InitWELLRNG19937a(init_seed);
	...
	*/
		...
		{
			int K = byte_popcount[/*buffer[i] ^*/ (*WELLRNG19937a)()];
			CC += K; K <<= 7;
